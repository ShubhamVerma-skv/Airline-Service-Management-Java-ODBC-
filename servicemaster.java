...
import java.sql.*;
public class servicemaster extends JPanel implements ActionListener
{
        JLabel lbid,lbservicecode,lbservicename,lbidshow;
        JTextField txtservicecode,txtservicename;
        ...
        servicemaster()
        {
                ...
                add("South",ps);
                lbid=new JLabel("ID");
                lbservicecode=new JLabel("Service Code");
                lbservicename=new JLabel("Service Name");
                lbidshow=new JLabel("");
                txtservicecode=new JTextField("");
                ...
                pn.add(lbid);
                pn.add(lbidshow);
                pn.add(lbservicecode);
                pn.add(txtservicecode);
                pn.add(lbservicecode);
                pn.add(txtservicecode);
                pn.add(lbservicename);
                pn.add(txtservicename);
                ps.add(btnadd);
                ps.add(btnsave);
                ps.add(btnnext);
                ps.add(btnprevious);
        ...
        void fillrecord()
        {
                try
                {
                        String s="select * from servicemaster";
        ...
        void showrecord()
        {
                try
                {
                        ...
                        txtservicecode.setText(rs.getString("servicecode"));
                        txtservicename.setText(rs.getString("servicename"));
                }
                catch(Exception e)
                {
                        JOptionPane.showMessageDialog(this,e.toString(),"airline",0);
        ...
        void saverecord()
        {
                try
                {
                        String cc=txtservicecode.getText();
                        String cn=txtservicename.getText();
                        String s="insert into servicemaster(servicecode,servicename)values('"+cc+"','"+cn+"')";
                        st.executeUpdate(s);
        ...
        void updaterecord()
        {
                try
                {
                        ...
                        String cc=txtservicecode.getText();
                        String cn=txtservicename.getText();
                        String s="update servicemaster set servicecode='"+cc+"',servicename='"+cn+"'where id="+id;
                        st.executeUpdate(s);
        ...
        void deleterecord()
        {
                try
                {
                        ...
                        {
                                int id=Integer.parseInt(lbidshow.getText());
                                String s="delete from servicemaster where id="+id;
                                st.executeUpdate(s);
                                fillrecord();
                                rs.next();
                                showrecord();
        ...
        {
                JFrame f=new JFrame("Service Master");
